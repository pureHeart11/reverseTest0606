...
const LeftArrow = ({ leftOption = [], switchState }) => {
  const leftTimer = useRef();
  const { positive, negative } = switchState;

  const arrowTo = (ctx, p1, p2, leftOptions) => {
    // 初始化参数
    let opts = {
    ...
    if (leftOptions !== undefined && leftOptions !== null) {
      opts = Object.assign(opts, leftOptions);
    }
  ...
  useEffect(() => {
    ...
    leftCanvas.style.cssText = 'position:absolute;left:0;top:0;'; // 画布样式

    // 画笔（绘图对象）
    let ctx = leftCanvas.getContext('2d');
    let BW = leftCanvas.width;
    let BH = leftCanvas.height;
    ctx.clearRect(0, 0, BW, BH); // 清空画布

    // 动画效果
    let _index = 1;
    if (!!positive) {
      leftTimer.current = setInterval(function () {
        // 高亮效果
        leftOption.map(({ startX, startY, endX, endY }) =>
  ...
  }, [positive, negative, leftOption, arrowTo]);

  // open

  return <canvas id="leftCanvas" width="300" height="600"></canvas>;
};
